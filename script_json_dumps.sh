#!/usr/bin/env bash
# json_dumps: timing + IPC/MPKI + DWARF record (aggregated) + flamegraph (+diff) + annotate
#* RUN AS ./json_perf_stats2.sh  OR  ./json_perf_stats2.sh after
...
# Warm-up helper (reduces marshal/r_object & import noise in later captures). Runs in a single process.
warmup_python() {
  python3 - <<'PY'
import json, sys
obj={"k":[1,2,3],"b":True,"s":"xyz"}
for _ in range(1000):
    json.dumps(obj)
PY
}
...
if [ "${RAW_JSON:-0}" -eq 1 ]; then
  # Raw steady-state runner (single process) for purer encoder profile
  python3 - <<'PY' > /dev/null 2>&1
import json, time
obj={"a":1,"b":True,"c":"val","d":[{"x":i} for i in range(20)]}
def run(n=200000):
    dumps=json.dumps
    for _ in range(n):
        dumps(obj)
run()  # measure loop executes here
PY
  PERF_STAT_CMD='python3 - <<"PY"\nimport json\nobj={"a":1}\nfor _ in range(200000): json.dumps(obj)\nPY'
else
  PERF_STAT_CMD="$CMD"
fi
...
for i in $(seq 1 "$FG_RUNS"); do
  rm -f perf.data
  if [ "${RAW_JSON:-0}" -eq 1 ]; then
    # Single-process tight loop (steady-state encoder)
    perf record -e cycles:u --call-graph dwarf,8192 -F "$FREQ" --mmap-pages 128 -- \
      python3 - <<'PY' || true
import json
obj={"a":1,"b":True,"c":"val","d":[{"x":i} for i in range(30)]}
dumps=json.dumps
...
./Flamegraph/flamegraph.pl --title "json_dumps • cycles:u (DWARF @ ${FREQ}Hz, ${FG_RUNS}x agg)" \
  "out${SUFFIX}.folded" > "flamegraph${SUFFIX}.svg" 2>/dev/null || true

# 5) Lightweight symbol summary (faster than full annotate; no disassembly)
if [ -f perf.data ]; then
