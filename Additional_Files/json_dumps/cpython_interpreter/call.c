...
/* Small-kwargs specialization helper (defined later) */
static int
_PyStack_UnpackDict_Small(PyThreadState *tstate,
                          PyObject *kwargs, Py_ssize_t nkwargs,
                          PyObject **kwstack, PyObject **p_kwnames);
                             ...
                             PyObject *kwargs)
{
    ...
    if (kwargs == NULL || PyDict_GET_SIZE(kwargs) == 0) {
        ...
        /*
           //* opt 5: Tiny-kwargs on-stack path (<= 4 keys), avoid heap array alloc.
           * Build vectorcall stack on stack; still allocate kwnames tuple.
           */
        Py_ssize_t nkwargs = PyDict_GET_SIZE(kwargs);
        if (nkwargs > 0 && nkwargs <= 4) {
            Py_ssize_t nargs = PyVectorcall_NARGS(nargsf);
            if (nargs + nkwargs <= (Py_ssize_t)_PY_FASTCALL_SMALL_STACK - 1) {
                PyObject *small_buf[_PY_FASTCALL_SMALL_STACK];
                PyObject **stack = small_buf + 1; /* for PY_VECTORCALL_ARGUMENTS_OFFSET */
                /* copy positional args with INCREF */
                ...
                if (_PyStack_UnpackDict_Small(tstate, kwargs, nkwargs, kwstack, &kwnames) < 0) {
                    while (nargs--) {
                        Py_DECREF(stack[nargs]);
                    }
                    return NULL;
                }

                res = func(callable, stack,
...
PyVectorcall_Call(PyObject *callable, PyObject *tuple, PyObject *kwargs)
{
    ...
    /*
       //* opt 1: Tiny-kwargs on-stack path (<= 4 keys), avoid heap array alloc.
       * Build vectorcall stack on stack; still allocate kwnames tuple.
       */
    if (kwargs != NULL) {
        Py_ssize_t nkwargs = PyDict_GET_SIZE(kwargs);
        if (nkwargs > 0 && nkwargs <= 4 && nargs + nkwargs <= (Py_ssize_t)_PY_FASTCALL_SMALL_STACK - 1) {
            PyObject *small_buf[_PY_FASTCALL_SMALL_STACK];
            PyObject **stack = small_buf + 1; /* for PY_VECTORCALL_ARGUMENTS_OFFSET */
            /* copy positional args with INCREF */
            PyObject **titems = _PyTuple_ITEMS(tuple);
            for (Py_ssize_t i = 0; i < nargs; i++) {
                Py_INCREF(titems[i]);
                stack[i] = titems[i];
            }
            ...
            PyObject *result = func(callable, stack,
...
/*
   //* opt 1: Small-kwargs specialization helper (<= 4 keys, no heap for args array).
   * Fills kwstack with INCREF'ed values and returns a freshly allocated
   * kwnames tuple with INCREF'ed keys. Caller must DECREF kwstack values and kwnames.
 */
static int
_PyStack_UnpackDict_Small(PyThreadState *tstate,
                          PyObject *kwargs, Py_ssize_t nkwargs,
                          PyObject **kwstack, PyObject **p_kwnames)
{
    ...
    Py_ssize_t pos = 0; Py_ssize_t i = 0;
    PyObject *key, *value;
    unsigned long keys_are_strings = Py_TPFLAGS_UNICODE_SUBCLASS;
    while (PyDict_Next(kwargs, &pos, &key, &value)) {
        keys_are_strings &= Py_TYPE(key)->tp_flags;
    ...
    if (!keys_are_strings) {
        ...
        /* Undo value INCREFs; tuple DECREF will drop keys. */
