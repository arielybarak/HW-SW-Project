...
#endif
//py_encode_basestring_ascii 697 encoder_listencode_obj 1537 encoder_listencode_dict 1686 encoder_listencode_list 1907
#include "Python.h"
#include "structmember.h"         // PyMemberDef
#include "pycore_accu.h"
#include <string.h>                 //* Enables memcpy for fast bulk copies
    ...
    */
    ...
    while (1) {
        ...
        for (next = end; next < len; next++) {
            ch = PyUnicode_READ(kind, buf, next);
            if (ch == '"' || ch == '\\') {
                break;
            }
            if (ch <= 0x1f && strict) {
        ...
        switch (ch) {
            case '"': case '\\': case '/':
                if (_PyUnicodeWriter_WriteChar(&writer, ch) < 0) goto bail;
                break;
            case 'b':
                if (_PyUnicodeWriter_WriteChar(&writer, '\b') < 0) goto bail;
                break;
            case 'f':
                if (_PyUnicodeWriter_WriteChar(&writer, '\f') < 0) goto bail;
                break;
            case 'n':
                if (_PyUnicodeWriter_WriteChar(&writer, '\n') < 0) goto bail;
                break;
            case 'r':
                if (_PyUnicodeWriter_WriteChar(&writer, '\r') < 0) goto bail;
                break;
            case 't':
                if (_PyUnicodeWriter_WriteChar(&writer, '\t') < 0) goto bail;
                break;
            case 'u': {
                /* Decode 4 hex digits */
                int i;
                Py_UCS4 c = 0;
                if (next + 4 > len) {
                    raise_errmsg("Invalid \\uXXXX escape", pystr, next - 2);
                    goto bail;
                }
                for (i = 0; i < 4; i++) {
                    Py_UCS4 digit = PyUnicode_READ(kind, buf, next++);
                    c <<= 4;
                    if ('0' <= digit && digit <= '9') c |= (digit - '0');
                    else if ('a' <= digit && digit <= 'f') c |= (digit - 'a' + 10);
                    else if ('A' <= digit && digit <= 'F') c |= (digit - 'A' + 10);
                    else {
                ...
                if (Py_UNICODE_IS_HIGH_SURROGATE(c)) {
                    if (next + 6 <= len &&
                        ...
                        Py_UCS4 c2 = 0;
                        int j;
                        Py_ssize_t save = next; /* position at '\\' */
                        next += 2; /* skip \\u */
                        if (next + 4 > len) {
                            raise_errmsg("Invalid \\uXXXX escape", pystr, save);
                            goto bail;
                        }
                        for (j = 0; j < 4; j++) {
                            Py_UCS4 digit = PyUnicode_READ(kind, buf, next++);
                            c2 <<= 4;
                            if ('0' <= digit && digit <= '9') c2 |= (digit - '0');
                            else if ('a' <= digit && digit <= 'f') c2 |= (digit - 'a' + 10);
                            else if ('A' <= digit && digit <= 'F') c2 |= (digit - 'A' + 10);
                            else {
                                raise_errmsg("Invalid \\uXXXX escape", pystr, next - 1);
                                goto bail;
                        ...
                        if (Py_UNICODE_IS_LOW_SURROGATE(c2)) {
                            ...
                            /* Not a low surrogate; backtrack to before the second escape */
                            next = save; /* let the next loop process the '\\' */
                        }
                    }
                }
                if (_PyUnicodeWriter_WriteChar(&writer, c) < 0) goto bail;
                break;
            }
            default:
                raise_errmsg("Invalid \\escape", pystr, next - 2);
                goto bail;
        }
        /* Continue scanning from current 'next' */
        end = next;
    }
...
static PyObject *
// purpose: Produces the JSON string form of a Python string, 
//      but restricted to ASCII (non‑ASCII gets escaped with \uXXXX).
py_encode_basestring_ascii(PyObject* Py_UNUSED(self), PyObject *pystr)
{
    PyObject *rval;
    /* METH_O */
    if (PyUnicode_Check(pystr)) {                   //Check if the string contains only ASCII
        
        if (PyUnicode_READY(pystr) == -1) {         //** Ensure the unicode object is ready before querying flags 
            return NULL;                            //"Ready"=> Unicode object's internal representation is fully computed and cached. CPython stores Unicode strings in different internal formats (1/2/4-byte), and PyUnicode_READY() ensures the most efficient representation is selected and flags like PyUnicode_IS_ASCII() are reliable to query.
        }                                           // The original code only calls ascii_escape_unicode() which handles this issue, but here we directly accesses Unicode flags/data, which require the object to be "ready" first.
        /*
           //* opt 1: ASCII-only, no-escapes fast path.
           * If the string is ASCII and all chars are S_CHAR, emit quotes and memcpy payload.
         */
        if (PyUnicode_IS_ASCII(pystr)) {
            Py_ssize_t len = PyUnicode_GET_LENGTH(pystr);
            const Py_UCS1 *src = PyUnicode_1BYTE_DATA(pystr);

            Py_ssize_t i = 0;
            int needs_escape = 0;                   //*Check if any character needs escaping - scan for any non S_CHAR (requires simple ASCII range check)
            while (i < len) {
            ...
            if (!needs_escape) {                    //* Fast path - just add quotes and memcpy the content
                Py_ssize_t outlen = len + 2;
                rval = PyUnicode_New(outlen, 127);
                if (rval == NULL) {
                    return NULL;
                }
                Py_UCS1 *dst = PyUnicode_1BYTE_DATA(rval);
                dst[0] = '"';
                if (len) {
                    memcpy(dst + 1, src, (size_t)len);
                }
                dst[outlen - 1] = '"';
                return rval;
            }
...
static int
//*Purpose: It’s the central “router” for encoding any Python object to JSON. 
// It decides: if it’s a str → encode as JSON string, dict → encode object, etc.. otherwise call the user’s default function
                       ...
                       PyObject *obj, Py_ssize_t indent_level)
{
    ...
    /*
       //* opt 3: Inline exact type fast paths using checks like PyUnicode_CheckExact, PyLong_CheckExact, etc. 
       These directly call the specific encoder for that type and return immediately, avoiding the more general path

       “Instance checks”: Asking “is this object a string? a dict?” at runtime (e.g., PyUnicode_Check, PyList_Check). 
       Exact-type checks are a cheaper subset (no subclass handling), so they’re faster and predictable
     */
...
static int
//* Purpose: Convert a Python dict into a JSON object
/*Emits “{” and “}”. Iterates key/value pairs. 
  Ensures keys are valid JSON strings (coercing supported key types to string form or skipping if skipkeys=True).
  Encodes values recursively (strings, numbers, arrays, objects, etc.).
  Handles separators ("," and ":") and optional sorted keys.
Why it matters: It’s the backbone for serializing Python dicts to JSON.
*/
                        ...
                        PyObject *dct, Py_ssize_t indent_level)
{
    ...
    /*
       //* opt 4: Unsored dicts: iterate with PyDict_Next to avoid building items list.
       * Preserve sorted semantics by using old path when sort_keys is True.

       PyDict_Next: A C API to iterate a Python dict without creating a list of items
       Why used: In unsorted dict path, it avoids building list(dct.items()) and the tuple allocations for each pair—less memory churn and faster iteration
     */
    if (s->sort_keys) {
        ...
        //* Fast path: iterate dict directly 
        Py_ssize_t pos = 0;
        PyObject *key, *value;
        idx = 0;
        while (PyDict_Next(dct, &pos, &key, &value)) {
            PyObject *encoded;
            /* Key handling mirrors the sorted path, but avoids tuple allocs */
            if (PyUnicode_Check(key)) {
                Py_INCREF(key);
                kstr = key;
            }
                        ...
                        PyObject *seq, Py_ssize_t indent_level)
{
    ...
    /*
       //* opt 4: Pre-emit separators using simple counter, no per-item branch for commas.
       //* opt 5: Inline simple-type fast paths; fallback to generic for others.
     */
    for (i = 0; i < PySequence_Fast_GET_SIZE(s_fast); i++) {
        PyObject *obj = PySequence_Fast_GET_ITEM(s_fast, i);
        if (i) {  // Pre-emit: simple counter-based separator, not complex branching
            if (_PyAccu_Accumulate(acc, s->item_separator))
                goto bail;
        }
        if (obj == Py_None || obj == Py_True || obj == Py_False) {
        ...
        if (PyUnicode_CheckExact(obj)) {
            ...
            if (encoded == NULL)
                goto bail;
            if (_steal_accumulate(acc, encoded))
                goto bail;
            continue;
        }
        if (PyLong_CheckExact(obj)) {
